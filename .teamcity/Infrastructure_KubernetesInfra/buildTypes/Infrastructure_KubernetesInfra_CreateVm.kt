...
object Infrastructure_KubernetesInfra_CreateVm : BuildType({
    ...
    steps {
        ...
        script {
            ...
            workingDir = "ansible/ansible-module-yandex-cloud"
            scriptContent = """
                cat > ansible.cfg <<EOF 
                %ansible.cfg%
                EOF
        ...
        script {
            name = "Create VM (workers)"
            workingDir = "ansible/ansible-module-yandex-cloud"
            scriptContent = """
                #!/bin/bash
                source ../venv/bin/activate
                export ANSIBLE_HOST_KEY_CHECKING=False
                ansible-playbook -i ../inventory.ini ../create-workers.yml --extra-vars 'yc_token="%infra.secrets.yc_token%" ssh_key="%infra.secrets.sshkey%"'
            """.trimIndent()
        }
    }
})
